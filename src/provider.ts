...
export function createInlineValuesProvider(typescript: typeof ts) {
  ...
  return function provideInlineValues(
    context: types.InlineValuesContext
  ): types.InlineValue[] {
    const ts = typescript;

    const { file, span, position } = context;

    const currentToken = ts.getTokenAtPosition(
      file as unknown as ts.SourceFile,
      position
    );
    ...
    function appendEvaluatableExpressionValue(
      expr: ts.Expression,
      ...
      if (ignorePosition || expr.end <= currentToken.pos) {
        ...
        const text = ts.usingSingleLineStringWriter((writer) =>
          printer.writeNode(
    ...
    function visitCaseOrDefaultClause(clause: ts.CaseOrDefaultClause) {
      if (ts.isCaseClause(clause)) {
        appendEvaluatableExpressionValue(
          clause.expression,
          /*ignorePosition*/ true
        );
      }
    ...
    function visitAssignmentLikeExpression(expr: AssignmentLikeExpression) {
      assert(
        isAssignmentLikeExpression(expr),
        "Must be assignment like expression."
      );
    ...
    function visitIfStatement(stmt: ts.IfStatement) {
      if (
        ...
        isAssignmentLikeExpression(stmt.expression.expression)
      ) {
    ...
    function visitForStatement(stmt: ts.ForStatement) {
      ...
      if (stmt.incrementor) {
         ...
         */
        if (
          isAssignmentLikeExpression(stmt.incrementor) ||
          (ts.isBinaryExpression(stmt.incrementor) &&
            stmt.incrementor.operatorToken.kind === ts.SyntaxKind.CommaToken)
        ) {
  ...
  function findScopes(node: ts.Node): ProvideableScope[] {
    ...
    ts.findAncestor(node, (node) => {
      if (
        scopeCount >= maxStrongScopeCount ||
        results.length >= maxScopeCount
      ) {
        return "quit";
      }
  ...
  function isAssignmentLikeExpression(
    expr: ts.Expression
  ): expr is AssignmentLikeExpression {
    const ts = typescript;

    return (
      ts.isAssignmentExpression(expr) || ts.isUnaryExpressionWithWrite(expr)
    );
  }
}
